...
// puuid -> ALL ranked league entries (solo/duo AND flex come back in the same
// response — tracking both queues costs zero extra calls). Primary path is
// league-v4 entries by-puuid; falls back through summoner-v4 for
// platforms/deploys where the by-puuid route isn't available.
export async function rankedEntries({ puuid, platform }) {
  const host = `https://${platform}.api.riotgames.com`;
  try {
    return (await riot(host, `/lol/league/v4/entries/by-puuid/${puuid}`)) || [];
  } catch (e) {
    if (e.status !== 404 && e.status !== 400) throw e;
    const summ = await getSummoner({ puuid, platform });
    return (
      (await riot(host, `/lol/league/v4/entries/by-summoner/${summ.id}`)) || []
    );
  }
}

// puuid -> ranked solo/duo entry | null (kept for lookupSoloRank / scripts).
