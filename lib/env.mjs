...
export function creds() {
  ...
  return {
    ...
    // Flex is always TRACKED (same API call, data kept current in .links.json)
    // but SILENT by default — Goonmaster 2026-07-30: "Let's ignore flex rank".
    // FLEX_VISIBLE=1 turns on flex log lines + celebrations; FLEX_ROLES=1 also
    // maintains a second "(Flex)" role ladder.
    flexVisible: !!process.env.FLEX_VISIBLE,
